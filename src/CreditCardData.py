...
class CreditCardData:
    ...
    def __init__(self, filepath="../data/creditcard.csv", split=False, test_prop=0.2):
        csv_data_og = np.loadtxt(open(filepath, "rb"),
                                 dtype=str, delimiter=",", skiprows=1, ndmin=2)
        self.data = np.array(csv_data_og[:, :-1]).astype(float)
        self.labels = np.array([row[-1].replace("\"", "")
                               for row in csv_data_og]).astype(int)
        ...
        if split:
            self.data_train, self.data_test, self.labels_train, self.labels_test = train_test_split(
                self.data, self.labels, test_size=test_prop)
    ...
    def oversample(self, X_train, y_train):
        ...
        for i in range(len(X_train)):
            # desired ratio minority/majority = 0.1
    ...
    def undersample(self, X_train, y_train):
        ...
        for i in range(len(X_train)):
            ...
            X, y = under.fit_resample(X_train[i], y_train[i])
            X_resampled.append(X)
            y_resampled.append(y)
        return  np.asarray(X_resampled), np.asarray(y_resampled)
